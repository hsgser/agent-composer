...
into). This test pins: the root is the namespaced child START; the terminal is the namespaced child
END carrying `commit_as == callsite`; and every node id is `ns`-prefixed under the callsite.
"""
...
def test_call_subgraph_matches_grow_call_shape():
    ...
    # Root == the namespaced child START (the sole seed point).
    assert sg.roots == [ns("c0", child.start_id)]
    # Terminal == the namespaced child END; its Output commits under the callsite.
